...
def rebuild_vba_modules(original: bytes, modules: list[tuple[str, Path, bool]]) -> bytes:
    """Rebuild a VBA project with exactly the supplied modules.

    ``modules`` contains ``(module_name, source_path, is_document_module)``
    entries.  The original project supplies the host references and project
    identity, while all module streams, directory records and workspace names
    are rebuilt so removed legacy modules cannot remain embedded.
    """
    ole = olefile.OleFileIO(io.BytesIO(original))
    project = VBA_Project(ole, "", "PROJECT", "VBA/dir")
    project.parse_project_stream()
    ...
    module_map = {module.name: module for module in project.modules}
    if not module_map:
        raise ValueError("The source VBA project has no modules")

    # The directory prefix contains references and project metadata.  Module
    # records are contiguous at the end of the stream and are replaced below.
    first_module = min(directory.index(record(0x19, name.encode("cp936")))
                       for name in module_map)
    prefix = bytearray(directory[:first_module])
    old_count = record(0x0F, struct.pack("<H", len(project.modules)))
    if prefix.count(old_count) != 1:
        raise ValueError("Unable to locate the VBA module count record")
    struct.pack_into("<H", prefix, prefix.index(old_count) + 6, len(modules))

    streams: dict[str, bytes] = {}
    directory = prefix
    project_lines = ole.openstream("PROJECT").read().decode("cp936").splitlines()
    metadata: list[str] = []
    for line in project_lines:
        if line.startswith("Document=") or line.startswith("Module="):
            continue
        if line == "[Workspace]":
            break
        metadata.append(line)
    insert_at = next((i for i, line in enumerate(metadata) if line.startswith("Name=")), len(metadata))
    declarations = ["Document=" + name + "/&H00000000" for name, _, document in modules if document]
    declarations += ["Module=" + name for name, _, document in modules if not document]
    metadata[insert_at:insert_at] = declarations
    metadata.append("[Workspace]")
    metadata.extend(name + "=0, 0, 0, 0, " for name, _, _ in modules)
    project_text = "\r\n".join(metadata) + "\r\n"

    for name, source_path, document in modules:
        source = source_path.read_text(encoding="utf-8").replace("\r\n", "\n").replace("\n", "\r\n")
        streams["VBA/" + name] = compress_vba(source.encode("cp936"))
        directory += module_record(name, document=document)
    directory += record(0x10, b"")
    streams["VBA/dir"] = compress_vba(bytes(directory))
    streams["VBA/_VBA_PROJECT"] = bytes.fromhex("cc61ffff000000")
    streams["PROJECT"] = project_text.encode("cp936")
