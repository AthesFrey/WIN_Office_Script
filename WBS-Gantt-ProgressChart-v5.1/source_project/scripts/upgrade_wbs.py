...
def upgrade(wbs, styles, strings):
    ...
    derived = {}
    def style(base, *, date=False, axis=False, weekend=False, wrap=False):
        key = (base, date, axis, weekend, wrap)
    ...
    def cell(r, c, val, st='0'):
        ...
        return node
    cell(1, 2, 'WBS Gantt Planner v5.1', '8'); row(1).set('ht', '28')
    ...
    for r, t in tasks.items():
        ...
        cell(r, 14, t['progress'], '11' if parent else '22')
    ...
    for r, t in tasks.items():
        ...
        for c in rows[r]:
            ...
            if start_col <= idx <= finish_col:
                c.set('s', '11' if parent else '22')
                if idx == start_col:
        ...
        ET.SubElement(pf, q('bgColor'), indexed='64')
        xf = deepcopy(xfs[11 if parent else 22]); xf.set('fillId', str(fill_id)); xf.set('applyFill', '1')
