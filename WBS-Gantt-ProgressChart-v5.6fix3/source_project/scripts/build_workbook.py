"""Build the English v5.6fix3 WBS/Gantt workbook from the legacy template."""
...
WORKBOOK_NAME = "WBS-Gantt-ProgressChart-v5.6fix3.xlsm"
PROGRESS_COLUMN_WIDTHS = (3.5, 12, 23, 29, 17, 30, 18, 10, 12, 10, 4)
NS = {
...
def button(parent, idx, label, x, y, width, macro, color="2563EB"):
    # Both toolbars use fixed worksheet coordinates (Excel: don't move or size
    # with cells). The anchor and shape transform must describe the same box.
...
def progress_drawing():
    ...
    # Rows 11-12 are the existing blank band above the fixed row-13 header.
    button(drawing, 100, "Undo (0)", 25, 235, 88, "UndoMilestones", "94A3B8")
    button(drawing, 101, "Redo (0)", 121, 235, 88, "RedoMilestones", "94A3B8")
    for level in range(1, 6):
        button(drawing, 101 + level, f"Delete Level {level}", 217 + (level - 1) * 108,
               235, 100, f"DeleteMilestoneLevel{level}", "B45309")
...
def add_styles(styles):
    ...
    ids = {"regular": style(regular), "title": style(title), "muted": style(light, wrap=True),
           ...
           "progress": style(regular, fill("EFF6FF"), num=10, align="center"),
...
def progress_sheet(ids):
    ...
    cols = sub(ws, "s", "cols")
    for i, width in enumerate(PROGRESS_COLUMN_WIDTHS, 1):
    ...
    # Reserve room around the fixed toolbars in the delivered worksheet.
    # Explicit heights keep the initial cell text clear of the buttons.
...
def build(source: Path, output: Path):
    ...
    # Document-scoped RibbonX: Excel removes these restrictions when this
    # workbook is deactivated. No global CommandBars customization is used.
    parts['customUI/customUI.xml'] = b'''<?xml version="1.0" encoding="UTF-8"?>
<customUI xmlns="http://schemas.microsoft.com/office/2006/01/customui">
  <commands>
    <command idMso="Undo" enabled="false"/>
    <command idMso="Redo" enabled="false"/>
    <command idMso="Repeat" enabled="false"/>
  </commands>
</customUI>'''
    package_rels = ET.fromstring(parts['_rels/.rels'])
    sub(package_rels, 'rel', 'Relationship', Id='rIdWbsHistoryUI',
        Type='http://schemas.microsoft.com/office/2006/relationships/ui/extensibility',
        Target='customUI/customUI.xml')
    parts['_rels/.rels'] = xml(package_rels)
    parts["xl/vbaProject.bin"] = rebuild_vba(
        parts["xl/vbaProject.bin"], ROOT / "src/ProgressChart.bas",
        {name: ROOT / "src" / (name + (".bas" if name == "Model_1" else ".cls"))
         for name in ["ThisWorkbook", "Model_1", "Sheet1", "Sheet4"]},
    )
    styles = ET.fromstring(parts["xl/styles.xml"])
    ids = add_styles(styles)
    # Remove the legacy helper sheets before adding Milestones.
    ...
    # intentionally kept as the data fixture; only the generated copy is
    ...
    drawing = ET.Element(q("xdr", "wsDr"), nsmap={"xdr": NS["xdr"], "a": NS["a"]})
    # Put history first so both buttons stay within the initial viewport.
    button(drawing, 2008, "Undo (0)", 25, 55, 88, "UndoWbs", "94A3B8")
    button(drawing, 2009, "Redo (0)", 121, 55, 88, "RedoWbs", "94A3B8")
    button(drawing, 2000, "Update Gantt", 217, 55, 108, "UpdateGantt")
    button(drawing, 2001, "Open Milestones", 333, 55, 124, "OpenProgressChart")
    button(drawing, 2002, "Import Selected Tasks", 465, 55, 155, "ImportSelectedMilestones", "0F766E")
    button(drawing, 2003, "Refresh All", 628, 55, 94, "RefreshAll", "7C3AED")
    button(drawing, 2004, "Add Task", 730, 55, 85, "AddTask", "2563EB")
    button(drawing, 2005, "Add Subtask", 823, 55, 104, "AddSubtask", "0F766E")
    button(drawing, 2006, "Promote (-1)", 935, 55, 103, "PromoteTasks", "475569")
    button(drawing, 2007, "Demote (+1)", 1046, 55, 103, "DemoteTasks", "475569")
