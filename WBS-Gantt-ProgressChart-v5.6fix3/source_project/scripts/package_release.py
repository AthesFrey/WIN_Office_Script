"""Package the v5.6fix3 workbook and reproducible source project with English paths."""
import argparse
import hashlib
...
def package(output: Path):
    files = {WORKBOOK_NAME: ROOT / 'dist' / WORKBOOK_NAME,
             ...
             'milestones_toolbar_preview.png': ROOT / 'dist/milestones_toolbar_preview.png'}
    for name in ['README.md', 'USAGE.md', 'CHECK_RESULTS.md', 'FIX3_NOTES.md']:
    ...
    for name in ['WBS.xlsm', 'pyproject.toml', 'uv.lock', 'README.md', 'USAGE.md', 'CHECK_RESULTS.md', 'FIX3_NOTES.md']:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=Path, default=ROOT.parent / Path(WORKBOOK_NAME).with_suffix('.zip'))
